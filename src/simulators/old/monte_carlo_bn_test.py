...
from src.estimators.factor_number.old.bai_ng_rework import IC1_fun, PC1_fun, BIC3_fun
...
estimator_num = 3 # hardcoding, depends on the structure of the function


def monte_carlo_nt(dims_tbl, iter_num, burning_period, r, SNR, rho, beta, k_max, simulation_seed):
    ...
    for x in range(0, dims_length):
        ...
        for e in range(0, iter_num):
            ...
            X = dgp_nt_correlations(Lambda = Lambda, N = dims_tbl[x, 0], T = dims_tbl[x, 1],
            ...
            T_mean = np.mean(X, axis=1).reshape(-1,1)  # time_mean -> column vector (one time mean for each of N observations)
            T_mean_matrix = np.ones([N, T]) * T_mean

            X = X - T_mean_matrix

            var_vec = np.var(X,axis = 1)

            X = X / np.sqrt(var_vec).reshape(-1,1)
            ...
            output_cube[x, 0, e] = IC1_fun(X, eigen_vecs_sort, k_max = k_max)[0]
            output_cube[x, 1, e] = PC1_fun(X, eigen_vecs_sort, k_max = k_max)[0]
            output_cube[x, 2, e] = BIC3_fun(X, eigen_vecs_sort, k_max = k_max)[0]
...
def monte_carlo_snr(dims_tbl, iter_num, burning_period,N , T, r, rho, beta, k_max, simulation_seed):
    ...
    for x in range(0, dims_length):
        ...
        for e in range(0, iter_num):
            ...
            # ------------
            # dividing ALWAYS done inside the function
            output_cube[x, 0, e] = IC1_fun(X, eigen_val_sort, k_max=k_max)[0]
            output_cube[x, 1, e] = PC1_fun(X, eigen_val_sort, k_max=k_max)[0]
            output_cube[x, 2, e] = BIC3_fun(X, eigen_val_sort, k_max=k_max)[0]
            output_cube[x, 3, e] = ED_fun(X, eigen_val_sort, k_max=k_max)[0]
            output_cube[x, 4, e] = ER_fun(X, eigen_val_sort, k_max=k_max)[0]
            output_cube[x, 5, e] = GR_fun(X, eigen_val_sort, k_max=k_max)[0]

            output_cube[x, 6, e] = TKCV_fun(X, fold_number=5, k_max=k_max)[0]
...
def monte_carlo_rho(dims_tbl, iter_num, burning_period, N, T, r, SNR, beta, k_max, simulation_seed):
...
def monte_carlo_beta(dims_tbl, iter_num, burning_period,N , T, r, SNR, rho, k_max, simulation_seed):
