...
with open(os.path.join(get_results_path(),"mc_run_log.txt"), "a") as text_file:
    print("log file for iid N growing", file = text_file)
...
# exporting
np.save(os.path.join(get_results_path(),"output_cube_iid_N_large"),output_cube_iid)
...
# exporting
results_iid_df.to_csv(os.path.join(get_results_path(),"results_iid_N_large.csv"), sep=",", index=False)
...
os.rename(os.path.join(get_results_path(),"mc_run_log.txt"),
          os.path.join(get_results_path(),"mc_run_log_iid_N_large.txt"))
