...
with open(os.path.join(get_results_path(),"mc_run_log.txt"), "a") as text_file:
    print("log file for _NTcorr_T_large", file = text_file)
...
np.save(os.path.join(get_results_path(),"output_cube_NTcorr_T_large"),output_cube_NTcorr)

avg_result_NTcorr = np.mean(output_cube_NTcorr,axis=2,keepdims=False)

results_NTcorr_df = df_maker(input_tbl,avg_result_NTcorr,["$N$","$T$","$TKCV$"])


results_NTcorr_df.to_csv(os.path.join(get_results_path(),"results_NTcorr_T_large.csv"), sep=",", index=False)
...
os.rename(os.path.join(get_results_path(),"mc_run_log.txt"),
          os.path.join(get_results_path(),"mc_run_log_NTcorr_T_large.txt"))
