...
# -------------------------------
from src.simulators.monte_carlo import monte_carlo_nt
...
print(f"log file for monte carlo nt_snr_{SNR}_rho_{rho}_beta_{beta}_Jtype_{J_type}_iter_num_{iter_num}")


with open(os.path.join(get_results_path(),"mc_run_log.txt"), "a") as text_file:
    print(f"log file for monte carlo nt_snr_{SNR}_rho_{rho}_beta_{beta}_Jtype_{J_type}_iter_num_{iter_num}", file = text_file)
...
# saving raw results
# ------------------
np.save(os.path.join(get_results_path(),f"output_cube_nt_snr_{SNR}_rho_{rho}_beta_{beta}_Jtype_{J_type}_iter_num_{iter_num}{suffix}"), output_cube_nt)
...
mean_stats_nt_df.to_csv(os.path.join(get_results_path(),
                 f"mean_stats_nt_snr_{SNR}_rho_{rho}_beta_{beta}_Jtype_{J_type}_iter_num_{iter_num}{suffix}.csv"), sep=",", index=False)

joined_stats_normal_nt_df.to_csv(os.path.join(get_results_path(),
                          f"joined_stats_normal_nt_snr_{SNR}_rho_{rho}_beta_{beta}_Jtype_{J_type}_iter_num_{iter_num}{suffix}.csv"), sep=",", index=False)

joined_stats_latex_nt_df.to_csv(os.path.join(get_results_path(),
                         f"joined_stats_latex_nt_snr_{SNR}_rho_{rho}_beta_{beta}_Jtype_{J_type}_iter_num_{iter_num}{suffix}.csv"), sep=",", index=False)
...
os.rename(os.path.join(get_results_path(),"mc_run_log.txt"),
          os.path.join(get_results_path(),f"mc_run_log_nt_snr_{SNR}_rho_{rho}_beta_{beta}_Jtype_{J_type}_iter_num_{iter_num}.txt"))
